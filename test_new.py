...
def feature_classifier(opt):
    ...
    sorted_exemplar_features = sortFeatures(features_exemplar_backbone, labels_examplar)
    ...
    features_testing_known_backbone_step2 = [features_testing_known_backbone[i] for i in range(len(features_testing_known_backbone)) and i not in step1_indices]
    labels_testing_known_step2 = [labels_testing_known1[i] for i in range(len(labels_testing_known1)) and i not in step1_indices]
    ...
    features_testing_unknown_backbone_step2 = [features_testing_unknown_backbone[i] for i in range(len(features_testing_unknown_backbone)) and i not in step1_indices_unknown]
    labels_testing_unknown_step2 = [labels_testing_unknown1[i] for i in range(len(labels_testing_unknown1)) and i not in step1_indices_unknown]
    ...
    auroc_step1 = AUROC(labels_binary_known_step1, probs_binary_step1, opt)
    print("AUROC Step1 is: ", auroc_step1)
    auroc_step2 = AUROC(labels_binary_known_step2, probs_binary_step2, opt)
    ...
    # OSCR
    oscr_step1 = OSCR(np.array(prediction_logits_known_dis_out_step1), np.array(prediction_logits_unknown_dis_out_step1), predictions_known_step1, labels_testing_known_step1)
    print("OSCR Step1 is: ", oscr_step1)
    oscr_step2 = OSCR(np.array(prediction_logits_known_dis_out_step2), np.array(prediction_logits_unknown_dis_out_step2), predictions_known_step2, labels_testing_known_step2)
    print("OSCR Step2 is: ", oscr_step2)

    #print("Acc Known: ", acc_known)

    return auroc_step1, auroc_step2             # oscr, acc_known
...
if __name__ == "__main__":
    ...
    auroc_step1, auroc_step2  = feature_classifier(opt)                        # oscr, acc_known
