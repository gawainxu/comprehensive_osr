...
class SupConLoss(nn.Module):
    ...
    def forward(self, features, labels=None, mask=None, features_negative = None, reduction='mean'):
        ...
        if features_negative is not None:
            anchor_dot_contrast_negative = torch.div(torch.matmul(anchor_feature_negative, contrast_feature_negative.T), self.temperature)
        ...
        # compute log_prob
        if features_negative is not None:
            exp_logits = torch.exp(logits_negative) * logits_mask
        else:
