...
class ModelBase(nn.Module):
    ...
    def __init__(self, figsize=32, num_classes=10, projection_dim=128, arch=None):
        ...
        self.net = resnet_arch(pretrained=False)
...
class MaskCon(nn.Module):
    def __init__(self, num_classes_coarse=6,  dim=128, K=4096, m=0.9, T1=0.05, T2=0.1, arch='resnet18', mode='mixcon', size=32):
        '''
    ...
    def initiate_memorybank(self, dataloader):
        ...
        for i in range(self.K):  # update the memory bank with image representation
            ...
            _, _, k = self.encoder_k(im_k_)  # keys: NxC
            k = nn.functional.normalize(k, dim=1)  # already normalized
            # undo shuffle
            k = self._batch_unshuffle_single_gpu(k, idx_unshufflek)
            self._dequeue_and_enqueue(k, coarse_label)

    def forward(self, im_k, im_q, coarse_label, w=0.5):
        with torch.no_grad():  # no gradient to keys
            self._momentum_update_key_encoder()
        cls_q, _, q = self.encoder_q(im_q)  # queries:
        ...
        with torch.no_grad():  # no gradient to keys
            ...
            # soft-labels
            # Similarities between keys and quene samples
        ...
        # in F.log_softmax(logits_all, 1), non-zeros should be as large as possible
        loss = -torch.sum(F.log_softmax(logits_all, 1) * labels.detach(), 1).mean()
        # inside vs outside?
        self._dequeue_and_enqueue(k, coarse_label)
        
        # just for visualization
        loss1 = -torch.sum(F.log_softmax(logits_all.detach(), 1) * maskcon_z.detach(), 1).mean()
        loss2 = -torch.sum(F.log_softmax(logits_all.detach(), 1) * self_z.detach(), 1).mean()
